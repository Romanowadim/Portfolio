...
type Props = {
  coworker?: Coworker;
  onClose: () => void;
  onSaved: (coworker: Coworker) => void;
  onDeleted?: (id: string) => void;
};

export default function CoworkerPickerModal({ coworker: editCoworker, onClose, onSaved, onDeleted }: Props) {
  const isEdit = !!editCoworker;

  const [name, setName] = useState(editCoworker?.name || "");
  const [role, setRole] = useState(editCoworker?.role || "");
  const [avatar, setAvatar] = useState(editCoworker?.avatar || "");
  const [socials, setSocials] = useState<{ icon: string; url: string }[]>(
    editCoworker?.socials || []
  );
  ...
  const handleSave = async () => {
    ...
    const coworker: Coworker = {
      id: editCoworker?.id || `coworker-${Date.now()}`,
      name: name.trim(),
      role: role || undefined,
      avatar: avatar || undefined,
      socials: socials.length > 0 ? socials.filter((s) => s.url) : undefined,
    };
    try {
  ...
  const handleDelete = async () => {
    if (!editCoworker || !onDeleted) return;
    if (!confirm("Удалить этого коворкера?")) return;
    setSaving(true);
    try {
      const res = await fetch("/api/coworkers", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: editCoworker.id }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        setError(data.error || `Error ${res.status}`);
      ...
      >
        ...
        >
          <h2 className="text-sm font-bold tracking-[2.8px] uppercase text-text-muted mb-8">
            {isEdit ? "РЕДАКТИРОВАТЬ КОВОРКЕРА" : "НОВЫЙ КОВОРКЕР"}
          </h2>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              {avatar ? (
                ...
                <ImageUpload onUploaded={setAvatar} compact />
              )}
            </div>
